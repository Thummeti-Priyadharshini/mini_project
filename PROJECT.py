    ...
    def cartoonify(self,ImagePath,x):
            ...
            if int(format(len(faces))) == 1:
                for (x, y, w, h) in faces:
                    ...
                    f = originalmage[y:y + h, x:x + w]
                gray1 = cv2.cvtColor(f, cv2.COLOR_BGR2RGB)

                R1 = cv2.resize(gray1, (960, 540))
                # converting an image to grayscale
                grayScaleImage = cv2.cvtColor(f, cv2.COLOR_BGR2GRAY)
...
class save_cartoon:
    ...
    def cartoon_save(self,E1,w3):
        if E1.get()=="":
            ...
            extension = '.jpg'
            path = os.path.join(path1, E1.get() + extension)
            cv2.imwrite(path, cv2.cvtColor(self.R6, cv2.COLOR_RGB2BGR))
